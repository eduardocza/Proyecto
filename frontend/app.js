...
class VulnerabilityTracker {
    ...
    async init() {
        ...
        document.getElementById('remediationsFile').addEventListener('change', (e) => {
            ...
            if (e.target.files[0]) {
                document.getElementById('remLabel').textContent = '✅ ' + e.target.files[0].name;
                document.getElementById('remBox').classList.add('loaded');
            }
        });
        document.getElementById('equipmentFile').addEventListener('change', (e) => {
            this.handleFileUpload(e, 'equipment');
            if (e.target.files[0]) {
                document.getElementById('eqLabel').textContent = '✅ ' + e.target.files[0].name;
                document.getElementById('eqBox').classList.add('loaded');
            }
        });
        document.getElementById('inventoryFile').addEventListener('change', (e) => {
            this.handleFileUpload(e, 'inventory');
            if (e.target.files[0]) {
                document.getElementById('invLabel').textContent = '✅ ' + e.target.files[0].name;
    ...
    handleFileUpload(event, type) {
        ...
        reader.onload = (e) => {
            ...
            else if (type === 'equipment') this.equipmentData = data;
            else if (type === 'inventory') this.inventoryData = data;
        };
    ...
    async processData() {
        ...
        try {
            ...
            // Si hay inventario nuevo, subirlo primero
            if (this.inventoryData && this.inventoryData.length > 0) {
                const devices = this.inventoryData.map(item => ({
                    name:       item['Nombre'] || item['nombre'] || item['Name'] || '',
                    owner:      item['USUARIO'] || item['Usuario'] || item['usuario'] || item['Owner'] || '',
                    department: item['DEPARTAMENTO'] || item['Departamento'] || '',
                    model:      item['MODELO'] || item['Modelo'] || item['modelo'] || ''
                })).filter(d => d.name);
    ...
    closeManualModal(event) {
        if (!event || event.target === document.getElementById('manualModal')) {
    ...
    fillEquipmentTable(tbodyId, items, prevItems) {
        ...
        items.forEach((item, i) => {
            ...
            row.insertCell(2).textContent = item.owner || '—';
            row.insertCell(3).textContent = item.vulnerabilities;
    ...
    async openModal(type) {
        ...
        try {
            ...
            if (type === 'remediations') {
                ...
                thead.innerHTML = `<tr><th>#</th><th>Equipo</th><th>Propietario</th><th>Vulnerabilidades</th><th>Estado</th></tr>`;
                tbody.innerHTML = '';
                data.equipment.forEach((item, i) => {
                    ...
                    row.insertCell(2).textContent = item.owner || '—';
                    row.insertCell(3).textContent = item.vulnerabilities;
                    row.insertCell(4).textContent = item.support_status || '—';
                });
            }
        } catch (err) {
