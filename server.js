...
app.get('/api/snapshots/latest', async (req, res) => {
    try {
        ...
        const top5Equipment = query(`
            SELECT id, name, owner, vulnerabilities, support_status
...
// ── POST /api/inventory ── cargar inventario desde CSV
app.post('/api/inventory', async (req, res) => {
    ...
    try {
        ...
        for (const d of devices) {
            run(`INSERT OR REPLACE INTO inventory (name, owner, department, model)
                 VALUES (?, ?, ?, ?)`,
                [d.name || '', d.owner || '', d.department || '', d.model || '']);
        }
...
// ── GET /api/inventory/status ── verificar si hay inventario cargado
app.get('/api/inventory/status', async (req, res) => {
    try {
        const { query } = await getDbModule();
        const count = query('SELECT COUNT(*) as total FROM inventory');
...
app.post('/api/snapshots', async (req, res) => {
    ...
    try {
        ...
        for (const e of (equipment || [])) {
            // Cruzar con inventario para obtener propietario
            const inv = query('SELECT owner FROM inventory WHERE name = ? COLLATE NOCASE LIMIT 1', [e.name || '']);
            const owner = inv.length > 0 ? inv[0].owner : '';
            run(`INSERT INTO equipment (snapshot_id, name, owner, vulnerabilities) VALUES (?, ?, ?, ?)`,
                [snapshotId, e.name || '', owner, e.vulnerabilities || 0]);
        }
